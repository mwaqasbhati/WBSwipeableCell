...
open class MenuView: UIView {
    ...
    fileprivate var stackViewTopRow = UIStackView(frame: .zero)
    fileprivate var stackViewBottomRow = UIStackView(frame: .zero)
    fileprivate var mCell: UIView?
    private var changableConstraint: NSLayoutConstraint!
    public private(set) var direction: Direction = .right
    fileprivate var menuLayout: MenuLayout = .square
    fileprivate var contentAlignment: ContentAlignment = .center
    fileprivate var menuButton = UIButton.init(type: .custom)
     ...
     */
    public convenience init(mCell: UIView, items: [MenuItem], gesture: Gesture? = .swipe, indexPath: IndexPath) {
     ...
     */
    public func open(from direction: Direction,withAnimation animated: Bool) {
     ...
     */
    public func close(withAnimation animated: Bool) {
        guard let changableConstraint = self.changableConstraint  else {
            return
        }
        _menuOpen = false
...
public extension MenuView {
     ...
     */
    fileprivate func setMenuContentSpacing(_ top: CGFloat, left: CGFloat, bottom: CGFloat, right: CGFloat) {
     ...
     */
    fileprivate func setMenuItemsHorizontalSpacing(_ spacing: CGFloat) {
        switch menuLayout {
        case .horizontal:
